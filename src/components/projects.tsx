...
const ProjectsContent = () => {
  ...
  useEffect(() => {
    ...
    void transitionToNewItems();

    async function transitionToNewItems() {
      ...
      // KEY: This keeps staying items in the same DOM positions but with new neighbors removed
      setDisplayedProjects(filtered);

      // Wait for React render
      await new Promise((resolve) => setTimeout(resolve, 50));

      // PHASE 4: FLIP and slide-in animations (with staying items already in new positions)
      if (listRef.current) {
        const newElements = Array.from(
          listRef.current.querySelectorAll("li"),
        ) as HTMLElement[];

        // First pass: Remove project-visible from staying items to reset them to off-screen state
        // This allows us to capture the true "Last" position before FLIP
      ...
      await new Promise((resolve) => setTimeout(resolve, maxAnimationDuration));
